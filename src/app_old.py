...
# --- Header ---
# Only show header if there are no user messages yet AND we're on the chatbot page
user_messages = [msg for msg in st.session_state['messages'] if msg['role'] == 'user']
if len(user_messages) == 0 and st.session_state.get('page', 'chatbot') == 'chatbot':
    st.markdown(
        """
...
# Only show chat history and form if we're on the chatbot page
if st.session_state.get('page', 'chatbot') == 'chatbot':
...
if st.session_state['page'] == 'internship':
    ...
    with col1:
        ...
        tasks = st.text_area("Tasks & Achievements", key="intern_tasks_minimal", height=120)
    ...
    if generate_doc and api_key:
        from src.models.llm_utils import IndustrialLLMHelper
        llm_helper = IndustrialLLMHelper(
...
"""
        with st.spinner("📝 Generating your internship documentation..."):
            doc = llm_helper.get_chat_response([
